...
// 파트너 로그인 폼: 이메일/비밀번호 + 구글 로그인, URL 파라미터 기반 안내 메시지 표시
...
export default function LoginForm({ googleEnabled = false }: { googleEnabled?: boolean }) {
  ...
  return (
    <form className="generate-box login-form-card" onSubmit={submit}>
      {googleEnabled ? (
        <>
          <a className="generate-button google-login-button" href="/api/auth/google">
          ...
          <p className="login-divider">또는 이메일로 로그인</p>
        </>
      ) : null}
